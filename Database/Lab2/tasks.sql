...
WHERE human."ИМЯ" = 'Александр' AND session."ЧЛВК_ИД" > 151200; -- можно заменить на 0 для хоть какого-то результата
...
WHERE human."ФАМИЛИЯ" < 'Ёлкин' AND edu."НЗК" > '001000';
...
WHERE student."ГРУППА" = '3102' AND human."ОТЧЕСТВО" != '.';
...
FROM "Н_УЧЕНИКИ" student JOIN "Н_ПЛАНЫ" plan ON student."ПЛАН_ИД" = plan."ИД"
WHERE EXTRACT(YEAR FROM student."НАЧАЛО") = '2011' AND plan."ОТД_ИД_ЗАКРЕПЛЕН_ЗА" = (SELECT "ИД" FROM "Н_ОТДЕЛЫ" WHERE "КОРОТКОЕ_ИМЯ" = 'ВТ')
...
SELECT (human."ФАМИЛИЯ" || ' ' || human."ИМЯ" || ' ' || human."ОТЧЕСТВО") AS "ФИО",
       avg."ИД",
       avg."Ср_оценка"
FROM MarksAverage avg JOIN "Н_ЛЮДИ" human ON avg."ЧЛВК_ИД" = human."ИД", MinAverage1101 min
