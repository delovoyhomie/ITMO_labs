...
public class RemoveGreaterKey extends Command {
    public RemoveGreaterKey() {
        super("remove_greater_key", "remove_greater_key {id} - удалить из коллекции все элементы, ключ которых превышает заданный");
    }
    ...
    public Response execute(Request request) {
        ...
        List<Integer> idsToRemove = request.getCollection().keySet().stream()
                .filter(k -> k > Integer.parseInt(request.getText()))
                .toList();

        idsToRemove.forEach(request.getCollection()::remove);
